  ...
  <div class="wrapper">
    ...
    <div class="userInfo-item-wrapper">
      ...
      <div class="userInfo-item" @click="() => navigatorToUserItem('tel', myUserInfo.tel || '')">
        ...
        <div class="item-right">{{ myUserInfo.tel || t('unknow') }}<Icon :size="15" color="#A6ADB6" iconClassName="arrow"
  ...
  uni.getNetworkType({
    success: function (res) {
      if (res.networkType === 'none') {
        ...
        uni.$UIKitStore.userStore.saveMyUserInfoActive({ ...myUserInfo.value, birth }).then(res => {
  ...
  uni.getNetworkType({
    success: function (res) {
      if (res.networkType === 'none') {
        ...
        uni.$UIKitStore.userStore.saveMyUserInfoActive({ ...myUserInfo.value, gender }).then(res => {
          myUserInfo.value = res
        }).catch(() => {
          uni.showToast({
            title: t('updateText') + t('genderText') + t('failText'),
  ...
  try {
    uni.chooseImage({
      ...
      success: function (res) {
        ...
        uni.$UIKitStore.userStore.saveMyUserInfoActive({ ...myUserInfo.value, type: 'image', filePath: res.tempFilePaths[0] }).then((res) => {
          myUserInfo.value = res
        }).catch(() => {
          uni.showToast({
