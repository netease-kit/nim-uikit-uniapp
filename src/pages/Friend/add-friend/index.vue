<template>
  ...
  <div class="search-input-wrapper">
    <Icon class="search-icon" :size="20" color="#A6ADB6" type="icon-sousuo"></Icon>
  ...
  <div class="user-wrapper" v-else-if="searchResState === 'searchResult'">
    ...
    <button v-if="relation !== 'stranger'" class="go-chat-button" :type="('primary' as any)" @click="gotoChat">{{
      $t('chatButtonText') }}</button>
    <button v-else class="go-chat-button" :type="('primary' as any)" @click="applyFriend">{{ $t('addText')
    }}</button>
  </div>
</template>
...
const userInfo = ref<UserNameCard>()
const relation = ref<Relation>('stranger')

autorun(() => {
  // 这行打印用于触发 autorun 执行，不能删除
  console.log('friends: ', store.uiStore.friends)
  if (userInfo.value?.account) {
    relation.value = store.uiStore
      .getRelation(
...
const handleSearch = async (event) => {
  try {
    const user = await store.userStore.getUserActive(event.detail.value)
    if (!user) {
